...
#Average spending amount grouped using family id, autorization number (each trip) and week (week 1, week 2 and week 3)
...
```
#no of item puchased from vendor

```{r}
ggplot(count_number_of_item_per_vendor, aes(x=Vendor_Number, y=No_of_item_per_vendor)) + 
    geom_point()
```
