...
```{r}
Redemption_data_df1<-sqldf("SELECT Report_Begin_Date,Report_End_Date,WIC_Fam_ID_Hash, Auth_Num_Hash,Avg(Requested_Amount) as Avg_Requested_Amount,Category_Code,Category_Name,Vendor_Name_x,Vendor_Number,Vendor_Name_y, Address, City, Zipcode
...
```{r}
Redemption_data_df1 <- Redemption_data_df1 %>%
...
```{r}
Redemption_data_df1$Avg_Requested_Amount<-format(round(Redemption_data_df1$Avg_Requested_Amount, 2), nsmall = 2) 
```
